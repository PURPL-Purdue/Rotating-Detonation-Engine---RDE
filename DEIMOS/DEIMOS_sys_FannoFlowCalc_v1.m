...
T = 283.15;             % Staic inlet temperature (K)
Rmid = 22.3524953;      % Average annulus diameter (mm)
...
epsilon = 0.015;                  % Stainless Steel surface roughness (mm) (CONSERVATIVE)
...
while err > tol && iter < max_iter
    ...
    % ---- Solve Fanno for inlet Mach ----
        ...
        fanno_fun = @(M) ...
            (1 - M^2)/(gamma(i)*M^2) + ((gamma(i)+1)/(2*gamma(i))) ...
            * log((M^2)/((2/(gamma(i)+1)) * (1 + (gamma(i)-1)/2*M^2))) - f(i)*L(i)/D(i);
